...
export class EstadisticaClientesComponent implements OnInit {
  title:string="Estadistica Clientes"
  Table:any
  ...
  cargarBarra(){
    ...
    this.mainService.getBarClientes(data)
                      .then(response => {
                        if(response){
                          let fortex:any = []
                          fortex.length = 0
                              ...
                              this.barChartLabels.forEach((element2,x) => {
                                if(element2==element.fecha){
                                  if(fortex.find((element1,i) => {
                                     return element1.label==element.nombre+' '+element.apellido
                                  })){
  ...
  cargarPie(){
    ...
    this.mainService.getPieClientes(data)
                      .then(response => {
                          let labels:any[] = []
                          let data:any[] = []
                          response.forEach((element,index) => {
